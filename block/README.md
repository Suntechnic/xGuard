# INTRODUCTION
xGuard Block - это простой в использовании инструмент для блокировки нежелательных посетителей на основе различных критериев, таких как IP-адрес, User-Agent и URI. Проект максимально простой и полностью созданный в процедурном стиле PHP, без использования фреймворков и сторонних библиотек.  
...
Для ротации логов добавьте в крон файл xGuard/block/agents/logrotator.php, с интервалом исполнения по вашему усмотрению. Пример для ежедневного запуска в полночь:  
```shell
0 0 * * * /usr/bin/php -f /путь_до_проекта/xGuard/block/agents/exchange.php
```